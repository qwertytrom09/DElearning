...
let currentGeneralExercise = null;
...
function loadDictionary() {
    ...
    if (saved) {
        window.dictionary = JSON.parse(saved);
...
function addToDictionary(wordId, wordData) {
    if (!window.dictionary.find(item => item.wordId === wordId)) {
        const date = new Date();
...
function startTest() {
    ...
    } else if (window.dictionary.length > 0) {
        ...
        button.addEventListener('click', function() {
            ...
            if (selectedContainer) {
                ...
                if (!selectedContainer.querySelector('.card')) {
                    populateContainer(selectedContainer, window.vocabularyData[topic], window.currentPages[topic], PAGE_SIZE, topic);
